...
public class TestTweetAPIClient {
    ...
    public void testUserCanTweet() {
        // 1. User send a tweet
        String tweet = "testing live from New York City! " + UUID.randomUUID().toString();
        ValidatableResponse response = this.tweetAPIClient.createTweet(tweet);
        // 2. Verify that the tweet was successfully send
        response.statusCode(200);
        String actualTweet = response.extract().body().path("text");
    ...
    public void testUserCanAgainDoTweetSuccessfully() {
        Response response = tweetAPIClient.sentTweet("Tweeting from New York!!!");
    ...
    public void testUserCannnotTweetDuplicateTweet() {
        Response response = tweetAPIClient.sentTweet("Hi this is Nafis" + UUID.randomUUID());
        response.getBody().prettyPrint();
        int statusCode = response.getStatusCode();
        String statusLine = response.getStatusLine();
        Assert.assertEquals(statusCode, 200);
    ...
    public void testUserCanSearchProfileSuccessfully() {
        Response response = tweetAPIClient.searchProfile("nafiszohaqa1");
        response.getBody().prettyPrint();
        int statusCode = response.getStatusCode();
        String statusLine = response.getStatusLine();
        Assert.assertEquals(statusCode, 200);
    ...
    public void testUser2CanSearchProfileSuccessfully() {
        Response response = tweetAPIClient.searchProfile("luckyshmuck");
        response.getBody().prettyPrint();
        int statusCode = response.getStatusCode();
        String statusLine = response.getStatusLine();
        Assert.assertEquals(statusCode, 200);
...
//        System.out.println();
    ...
    private ResponseAwareMatcher<Response> hasItems(String michael, String lindsay) {
        return null;
    }
...
*/
    ...
    public void badCallAllEmployeeResources() {
        ...
        try {
            String baseUrl = null;
            response = given().when().get(null + "/api/AllEmployeeResources809879");
